...
# call bmtcom.sp_tmultidtbroadnogoods_upload ('dep_sub_date('YYYY-MM-DD 00:00:00', 1, days)', 'dep_add_date('YYYY-MM-DD 00:00:00', 2, days)', '#{dep_s3_url}' );
    ...
    querys = ["SET TIME ZONE 'Asia/Seoul';",
              "call bmtcom.sp_tmultidtbroadnogoods_upload ('{}', '{}', 's3://{}/{}');".format(start_datetime, end_datetime, s3url, s3_key)]
...
def truncate_tables(**kwargs):
    hook = RedshiftSQLHook(redshift_conn_id=redshift_conn_id)
    # SQL 쿼리 정의
    querys = ["truncate bmtcom_wk.tmultidtbroadnogoods_wk;",]
    for query in querys:
        hook.run(query)
...
with DAG(
    dag_id="dags_python_with_redshiftsqlhook_tmultidtbroadnogoods",
    default_args=default_args,
    description="Redshift에서 데이터를 SQLAlchemy 엔진으로 가져오는 DAG",
    schedule=[dataset_dags_oracle_s3_tmultidtbroadnogoods],
