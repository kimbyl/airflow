...
def call_stored_procedure(**kwargs):
    ...
    querys = ["SET TIME ZONE 'Asia/Seoul';",
              f"""
                delete from stoa_on.m_vision_pgm_day_view_order_time_daily
                where bd_date =  '{end_date}';
              """,
              f"""
                insert into stoa_on.m_vision_pgm_day_view_order_time_daily ( bd_date
                                                           , bd_btime
                                                           , tape_code
                                                           , base_mi
                                                           , dif_order_time_cd
                ...
                select c.bd_date
                    ...
                    , case
                        when a.dif_order_time < 1 then '0.1초미만'
                        when a.dif_order_time >= 1 and a.dif_order_time < 10 then '1.1-10초'
                        when a.dif_order_time >= 10 and a.dif_order_time < 30 then '2.10-30초'
                        when a.dif_order_time >= 30 and a.dif_order_time < 60 then '3.30초-1분'
                        when a.dif_order_time >= 60 and a.dif_order_time < 120 then '4.1분-2분'
                        when a.dif_order_time >= 120 and a.dif_order_time < 180 then '5.2분-3분'
                        when a.dif_order_time >= 180 and a.dif_order_time < 240 then '6.3분-4분'
                        when a.dif_order_time >= 240 and a.dif_order_time < 300 then '7.4분-5분'
                        when a.dif_order_time >= 300 then '8.5분이상'
                    end as      dif_order_time_cd
                    , count(*) order_cnt
                from (select log_dt
                        , pn
                        , sid
                        ...
                        , min(case when ec_type in ('begin_checkout', 'checkout_progress') then start_time end) as order_time
                        , datediff('second', min(start_time),
                                    nvl(min(case when ec_type in ('begin_checkout', 'checkout_progress') then start_time end),
                                        min(start_time)))                                                        as dif_order_time
...
with DAG(
    dag_id="dags_python_with_redshiftsqlhook_m_vision_pgm_day_view_order_time_daily",
