...
def call_stored_procedure(**kwargs):
    ...
    start_datetime = (kwargs.get('data_interval_end').in_timezone('Asia/Seoul') + relativedelta.relativedelta(days=-1)).strftime("%Y-%m-%d")
    end_datetime = kwargs.get('data_interval_end').in_timezone('Asia/Seoul').strftime("%Y-%m-%d")

    hook = RedshiftSQLHook(redshift_conn_id=redshift_conn_id)

    # SQL 쿼리 정의
    querys = ["SET TIME ZONE 'Asia/Seoul';",
              "call stoa_log.sp_f_cslog_daily_history_upload_loop ('{}', '{}', '{}');".format(start_datetime, end_datetime, end_datetime)]
...
with DAG(
    dag_id="dags_python_with_redshiftsqlhook_sp_f_cslog_daily_history_upload_loop",
    default_args=default_args,
    description="Redshift에서 데이터를 SQLAlchemy 엔진으로 가져오는 DAG",
    schedule='0 7 * * *',
    start_date=pendulum.datetime(2024, 11, 26, tz='Asia/Seoul'),
    catchup=False
) as dag:
