...
class CustomPostgressHook(BaseHook):
    ...
    def bulk_load(self, table, file, delimiter: str, is_header: bool, is_replace: bool):
        from sqlalchemy import create_engine

        self.log.info('적재 대상파일: ' + file)
        self.log.info('테이블: ' + table)
        self.get_conn()
        header = 0 if is_header else None
        if_exists = 'replace' if is_replace else 'append'
        file_df = pd.read_csv(file, header=header, delimiter=delimiter)

        for col in file_df.columns:
            try:
                if isinstance(file_df[col], str):
                    file_df[col] = file_df[col].str.replace('\r\n', '')
                    self.log.info(f'{table}.{col}: 개행문자 제거')
            except:
                continue
